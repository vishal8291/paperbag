  ...
  useEffect(() => {
    ...
    fetch(`${base}/api/products/${params.id}`)
      ...
      .catch(() => {
        ...
        router.push(`/store/${params.slug}/products`);
      })
      ...
      <div className="min-h-screen" style={{ background: "var(--cream)" }}>
        <div className="max-w-6xl mx-auto px-6 py-10">
          ...
          <nav className="flex items-center gap-2 text-sm mb-8" style={{ color: "#9ca3af" }}>
            <Link href={`/store/${params.slug}`} className="hover:text-green-700 transition">Home</Link>
            <span>/</span>
            <Link href={`/store/${params.slug}/products`} className="hover:text-green-700 transition">Products</Link>
            <span>/</span>
